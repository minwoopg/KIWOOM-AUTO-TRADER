...
class MarketRegimeClassifier:
    ...
    def classify(self, bars: list[PriceBar]) -> tuple[MarketRegime, str]:
        ...
        # ── NEUTRAL 판정 ──────────────────────────────────────────
        # 추세가 애매하지만 반등/눌림목 매수는 허용하는 구간
        # RSI 35~65 + 극단적 신호 없음 → NEUTRAL

        rsi_neutral = 35 <= rsi <= 65

        # MA 상승 + RSI 하락 + MACD 데드크로스 → 모멘텀 약화, NEUTRAL
        if ma_bullish and rsi_falling and macd_dead and rsi_neutral:
            return MarketRegime.NEUTRAL, (
                f"NEUTRAL — MA 상승({short_ma:,.0f}>{long_ma:,.0f}) "
                f"+ RSI {rsi:.1f}{rsi_dir_tag} 하락 + MACD 데드크로스({macd_line:+.1f}) "
                f"— 모멘텀 약화, 반등/눌림목 매수만 허용"
            )

        # MA 하락 + RSI 상승 + MACD 골든크로스 → 반등 시도, NEUTRAL
        if ma_bearish and rsi_rising and macd_golden and rsi_neutral:
            return MarketRegime.NEUTRAL, (
                f"NEUTRAL — MA 하락({short_ma:,.0f}<{long_ma:,.0f}) "
                f"+ RSI {rsi:.1f}{rsi_dir_tag} 상승 + MACD 골든크로스({macd_line:+.1f}) "
                f"— 반등 시도, 반등/눌림목 매수만 허용"
            )

        # MA 상승 + MACD 데드크로스 + RSI 중립 → 전환 준비, NEUTRAL
        if ma_bullish and not rsi_overbought and macd_dead and rsi_neutral:
            return MarketRegime.NEUTRAL, (
                f"NEUTRAL — MA 상승({short_ma:,.0f}>{long_ma:,.0f}) "
                f"+ MACD 데드크로스({macd_line:+.1f}) RSI {rsi:.1f}{rsi_dir_tag} "
                f"— 전환 준비 중, 반등/눌림목 매수만 허용"
            )
        ...
        if rsi_oversold and rsi_rising:
            return MarketRegime.NEUTRAL, (
                f"NEUTRAL — RSI {rsi:.1f}{rsi_dir_tag} 과매도 + 반등 중 "
                f"— 반등/눌림목 매수만 허용"
            )

        # 그 외 RSI 중립 구간 → NEUTRAL
        if rsi_neutral:
            return MarketRegime.NEUTRAL, (
                f"NEUTRAL — MA {short_ma:,.0f}/{long_ma:,.0f} "
                f"RSI {rsi:.1f}{rsi_dir_tag} MACD {macd_line:+.1f} "
                f"— 추세 불명확, 반등/눌림목 매수만 허용"
            )

        # 그 외 모든 경우 → SIDEWAYS (완전 관망)
        return MarketRegime.SIDEWAYS, (
            f"MA {short_ma:,.0f}/{long_ma:,.0f} "
            f"RSI {rsi:.1f}{rsi_dir_tag} "
            f"MACD {macd_line:+.1f} — 추세 불명확, 관망"
        )
