...
# 2026-07-28 (GPT 코드리뷰 지적, 8차): 시나리오 6이 TradingService.
# _try_buy()를 직접 호출하는데, 14:50 KST 게이트(now_kst() 기반)를
# 시각 고정 없이 두면 실행 시각에 따라 결과가 달라짐(재현 확인 —
# KST 14:50 이후 실행 시 AFTER_1450으로 먼저 차단되어 시나리오
# 자체가 검증하려는 재매수 로직에 도달 못 함). 장중 KST로 고정.
